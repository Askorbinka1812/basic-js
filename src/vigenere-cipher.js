  ...
  encrypt(message, key) {
    ...
    while (i < message.length) {
      if (alp.indexOf(message[i]) != -1) {
        message[i] = alp[(alp.indexOf(message[i]) + alp.indexOf(key[j])) % alp.length];
        j = ++j % key.length;
      }
      i++;
    }
  ...
  decrypt(encryptedMessage, key) {
    ...
    while (i < encryptedMessage.length) {
      if (alp.indexOf(encryptedMessage[i]) != -1) {
        encryptedMessage[i] = alp[(alp.indexOf(encryptedMessage[i]) + alp.length - alp.indexOf(key[j])) % alp.length];
        j = ++j % key.length;
      }
      i++;
    }
    return (this.stat) ? encryptedMessage.join('') :  encryptedMessage.reverse().join('');
  }
};
